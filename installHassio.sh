...
URL_BIN_APPARMOR="${URL_RAW_BASE}/usr/sbin/hassio-apparmor" #SI
URL_BIN_HASSIO="${URL_RAW_BASE}/usr/sbin/hassio-supervisor" #SI
URL_DOCKER_DAEMON="${URL_RAW_BASE}/etc/docker/daemon.json" #SI
URL_HA="${URL_RAW_BASE}/usr/bin/ha" #SI
URL_INTERFACES="${URL_RAW_BASE}/etc/network/interfaces" #SI
URL_NM_CONF="${URL_RAW_BASE}/etc/NetworkManager/NetworkManager.conf" #SI
URL_NM_CONNECTION="${URL_RAW_BASE}/etc/NetworkManager/system-connection/default" #SI
URL_SERVICE_APPARMOR="${URL_RAW_BASE}/etc/systemd/system/hassio-apparmor.service" #SI
URL_SERVICE_HASSIO="${URL_RAW_BASE}/etc/systemd/system/hassio-supervisor.service" #SI
URL_APPARMOR_PROFILE="https://version.home-assistant.io/apparmor.txt" 

# Chequea env, si falta lo avisara
...
if [ ! -z "${MISSING_PACKAGES}" ]; then
    warn "Lo siguiente falta en el host y necesita "
    warn "para ser instalado y configurado antes de ejecutar este script nuevamente"
    error "Faltan: ${MISSING_PACKAGES[@]}"
fi

# Revisa si Modem Manager is enabled
if systemctl is-enabled ModemManager.service &> /dev/null; then
    warn "ModemManager service esta activado. Esto puede causar problemas al usar dispositivos en serie."
fi

# Detectar la configuración incorrecta del registrador de Docker
if [ ! -f "$FILE_DOCKER_CONF" ]; then
  ...
  info "Reiniciando docker service"
...
info "Creando configuracion NetworkManager"
